...
class CSP2DApp:
    ...
    def init_CSP(self):
        ...
        self.crystal = UnitCell(frame_center=FRAME_CENTER)
        ...
        self.graphic_cell = self.canvas.create_rectangle(self.uc_bounds[0][0], self.uc_bounds[1][1],
                                     self.uc_bounds[0][1], self.uc_bounds[1][0], outline="white")
        self.text = self.canvas.create_text(100, 10, text="Energy: ???", fill="white")
        ...
        for atom in self.crystal.atoms:
            self.c_atoms.append(self.canvas.create_oval(
                ...
                fill=atom.colour, outline="white"
            ))
        ...
        for image in self.crystal.images:
            self.c_images.append(self.canvas.create_oval(
                ...
                fill=image.colour, outline="white"
            ))

        self.old_energy = 99999999999
        self.new_energy = 0
        self.energy_dif = 1

        self.take_step(0, 1000)


    def take_step(self, num_steps, max_steps):
        if num_steps < max_steps and abs(self.energy_dif) > 0.001:
            ...
            self.energy_dif = self.new_energy - self.old_energy
